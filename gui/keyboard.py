...
class Keyboard:
  def __init__(self):
    ...
    print(KeyCode.from_char('a'))

    # for key in KeyCode: print(key)
    ...
    # self.buttons[key].setStyleSheet("font-size:15px; border-radius: 5px; background-color: rgb(170, 170, 170)")
   

    # self.buttons[key].setStyleSheet("font-size:15px; border-radius: 5px; background-color: rgb(230,230,230)")
  ...
  def reportProgress(self, b):
    ...
    while (i < len(s)):
      if s[i] == "PRESS":
        ...
        # self.buttons[s[i+1]].setText("123")
  ...
  def display(self):
    ...
    x = 0
    y = 0
    for rows,rows2 in zip(self.keys, self.actual_keys):
      x = 0
